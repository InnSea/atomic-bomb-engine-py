...
从 0.41.4 版本开始，BatchRunner 提供了 `stop()` 方法，可以立即停止正在运行的压测。
...
更多使用示例请参考 `example_with_stop.py` 文件。
...
## [0.41.4] - 2024-10-21
### Added
- 新增 `stop()` 方法，支持立即停止正在运行的压测
  - 可以在任何时候调用 `runner.stop()` 来停止压测
  - 支持基于条件的停止（如错误数阈值、时间限制等）
  - 支持信号处理器优雅停止（Ctrl+C）
  - 详细使用方法请参考 `DEVELOPMENT.md` 和 `example_with_stop.py`
