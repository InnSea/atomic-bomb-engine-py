...
async def test_data_pool_random():
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    csv_path = os.path.join(script_dir, "pertest_275_user.csv")
...
def test_data_pool():
    ...
    runner.run(
        ...
        api_endpoints=[
            abe.endpoint(
                name="表单提交",
                url="http://127.0.0.1:7788/api/ai/test/v1",
                method="POST",
                weight=1,
                json={
                    "name": "{{username}}",
                ...
                assert_options=[
                    abe.assert_option(jsonpath="$.data.name", reference_object="Test01")
                ]
            )
        ],
        verbose=False,
